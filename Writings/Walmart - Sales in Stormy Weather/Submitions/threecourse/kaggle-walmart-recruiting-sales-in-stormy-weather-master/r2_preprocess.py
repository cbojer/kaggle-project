...
def preprocess(_df, is_train):
    ...
    wtr["depart_flag"] = 0.0
    wtr["depart_flag"] = np.where(
        wtr["depart2"] < -8.0, -1, wtr["depart_flag"])
...
valid_store_items = set(
    zip(store_item_nbrs.store_nbr, store_item_nbrs.item_nbr))

# preprocess
df_train = pd.read_csv("data/train.csv")
mask_train = [(sno_ino in valid_store_items)
              for sno_ino in zip(df_train['store_nbr'], df_train['item_nbr'])]
df_train = df_train[mask_train].copy()
preprocess(df_train, True).to_pickle('model/train2.pkl')

df_test = pd.read_csv("data/test.csv")
mask_test = [(sno_ino in valid_store_items)
             for sno_ino in zip(df_test['store_nbr'], df_test['item_nbr'])]
