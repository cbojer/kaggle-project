...
for(idx in 1:nrow(data_info)) {
  ...
  if(adjusted_weekly_daily_frequency == TRUE) {
    ...
    # Define save path
    save_path <- paste0(khs_feat_save_path, "adjusted/", id, ".RDS")
  } else {
    # Check directory existence
    ...
    # Define save path
    save_path <- paste0(khs_feat_save_path, "original/", id, ".RDS")
  }
  ...
  if(nrow(incomplete_feats) != 0) {
    ...
    for(retry_i in 1:khs_number_of_retry) {
      writeLines(sprintf("Retry %d of %d", retry_i, khs_number_of_retry))
      ...
      if(adjusted_weekly_daily_frequency == TRUE) {
        ...
        # Define save path
        retry_save_path <- paste0(khs_retry_save_path, "adjusted/", id, ".RDS")
      } else {
        # Check directory existence
        ...
        # Define save path
        retry_save_path <- paste0(khs_retry_save_path, "original/", id, ".RDS")
      }
      
      # Save Retry Data to Cache
      saveRDS(retry_data, paste0(retry_save_path, id, "_retry", retry_i, ".RDS"))
      ...
      retry_ids <- retry_data[, .GRP, c(group)][, ..group]
  ...
  # Save result
  saveRDS(cbind(id = id, complete_feats), save_path)
...
# Load data
M3 <- readRDS(processed_data_paths[grep("M3DT", processed_data_paths)])
M4 <- readRDS(processed_data_paths[grep("M4DT", processed_data_paths)])
...
# Save results
saveRDS(M3_feats, paste0(save_path, "M3.RDS"))
saveRDS(M4_feats, paste0(save_path, "M4.RDS"))
