...
if(!"M4comp2018" %in% installed.packages()) {
  ...
  remotes::install_github("carlanetto/M4comp2018")
}
...
for(idx in 1:nrow(data_info)) {
  ...
  save_path <- paste0(processed_training_save_path, id, ".RDS")
  ...
  if(length(train_data) > 1) {
    ...
    train <- rbindlist(lapply(raw_data[train_data], function(dt) {
      ...
      if(length(ids) == length(c(group, index, target))) {
        ...
        tmp <- suppressor(melt(dt, id.vars = as.character(ids), variable.factor = FALSE, na.rm = TRUE))
  ...
  train[, c(index) := lapply(.SD, as.IDate, format = "%Y-%m-%d"), .SDcols = c(index)]
  invisible(gc())
  ...
  writeLines(sprintf("\t Removing leading and trailing zero's from Train"))
  train <- train[, trimlt(.SD, target), by = c(group), .SDcols = c(target, index)]
...
# Repackage competition data in tidy data.table format.
...
# Save repackaged data
saveRDS(M3DT, paste0(processed_training_save_path, "M3DT.RDS"))
saveRDS(M4DT, paste0(processed_training_save_path, "M4DT.RDS"))
